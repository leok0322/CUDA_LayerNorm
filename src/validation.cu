...
int main(int argc, char **argv) {
    ...
    float *mean = nullptr; float *rstd = nullptr; float *weight = nullptr; float *bias = nullptr;
    ...
    mean    = static_cast<float *>(malloc(sizeof(float) * max_size));
    rstd    = static_cast<float *>(malloc(sizeof(float) * max_size));
    weight  = static_cast<float *>(malloc(sizeof(float) * max_size));
    bias    = static_cast<float *>(malloc(sizeof(float) * max_size));
    ...
    memset(mean,    0, sizeof(float) * max_size);
    memset(rstd,    0, sizeof(float) * max_size);
    std::fill(weight, weight + max_size, 1.0f);
    std::fill(bias, bias + max_size,     0.0f);

    // ── GPU 内存分配与数据传输 ────────────────────────────────────────────────
    float *dA = nullptr, *dout = nullptr, *dout_ref = nullptr;
    float *dmean = nullptr; float *drstd = nullptr; float *dweight = nullptr; float *dbias = nullptr;
    ...
    cudaCheck(cudaMalloc(reinterpret_cast<void **>(&dmean),   sizeof(float) * max_size));
    cudaCheck(cudaMalloc(reinterpret_cast<void **>(&drstd),   sizeof(float) * max_size));
    cudaCheck(cudaMalloc(reinterpret_cast<void **>(&dweight), sizeof(float) * max_size));
    cudaCheck(cudaMalloc(reinterpret_cast<void **>(&dbias),   sizeof(float) * max_size));
    ...
    cudaCheck(cudaMemcpy(dmean,   mean,  sizeof(float) * max_size,            cudaMemcpyHostToDevice));
    cudaCheck(cudaMemcpy(drstd,   rstd,  sizeof(float) * max_size,            cudaMemcpyHostToDevice));
    cudaCheck(cudaMemcpy(dweight, weight,sizeof(float) * max_size,            cudaMemcpyHostToDevice));
    cudaCheck(cudaMemcpy(dbias,  bias,   sizeof(float) * max_size,            cudaMemcpyHostToDevice));
    ...
    for (uint msize : mSIZE) {
        for (uint nsize : nSIZE) {
            ...
            if (kernel_num != 0) {
                try {
                    run_kernel(0,          m, n, dA, dout_ref, dmean, drstd, dweight, dbias); // 参考 kernel（base）
                    run_kernel(kernel_num, m, n, dA, dout,dmean, drstd, dweight, dbias); // 待测 kernel
            ...
            for (int j = 0; j < repeat_times; j++) {
                try {
                    run_kernel(kernel_num, m, n, dA, dout,dmean, drstd, dweight, dbias);
                } catch (const std::exception &e) {
                    fprintf(stderr, "%s\n", e.what());
                }
            }
            ...
            // ── FLOPS 计算 ────────────────────────────────────────────────────────
            // LayerNorm 每行 n 个元素的操作数估算（4 pass）：
            //   Pass1（求均值）     : n 次加法 + 1 次除法                ≈ n
            //   Pass2（求方差）     : n 次减法 + n 次乘法 + n 次加法
            //                        + 1 次除法                          ≈ 3n
            //   Pass3（求 rstd）    : 1 次加法 + 1 次 sqrt + 1 次倒数    ≈ 3（可忽略）
            //   Pass4（归一化+仿射）: n 次减法 + n 次乘法（×rstd）
            //                        + n 次乘法（×γ）+ n 次加法（+β）    = 4n
            //   每行合计：8n 次；m 行总计：8n × m
            // 注：Pass3 的 3 次操作为常数，大规模时可忽略不计
            // 注：原为 uint 运算后赋给 long，N≥2048 时超出 UINT_MAX 发生截断，改为 long long
            long long floatPointOperations = static_cast<long long>(8 * n) * m;
    ...
    cudaCheck(cudaFree(dmean));
    cudaCheck(cudaFree(drstd));
    cudaCheck(cudaFree(dweight));
